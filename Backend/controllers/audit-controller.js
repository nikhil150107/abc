  ...
  try {
    ...
    for (const [key, group] of logGroupMap.entries()) {
      ...
      const violationDoc = {
        ...
        breachLocation: {
          ...
          columns: ['message', 'rawLogData', ...group.detectedTypes],
          endpoint: group.action ? `/${group.action.toLowerCase()}` : '/app-logs',
          handler: group.action === 'USER_REGISTERED_SIMULATION' ? 'userController.js:registerUser' : 'auditFeedController.js',
          component: 'Server Logging Engine & Audit Table (MySQL: application_audit_logs)',
          codeReference: 'DemoApp/backend/src/utils/logger.js -> application_audit_logs table'
        },
        occurrences: group.count,
        title: `Plaintext Personal Data (${group.detectedTypes.join(', ')}) in ${group.action} Logs`,
        reason: `Application log recorded unmasked personal data [${group.detectedTypes.join(', ')}] across ${group.count} log entries in table 'application_audit_logs'. DPDPA Section 8(5) mandates reasonable security safeguards to prevent unauthorized personal data exposure in log files.`,
    ...
    if (purposeMismatchEvents.length > 0) {
      ...
      const violationDoc = {
        ...
        detectedPII: ['PHONE'],
        breachLocation: {
          originType: 'DATABASE_TABLE / API',
          table: 'orders, order_items, data_processing_events',
          columns: ['shippingAddress', 'shippingCity', 'mobileNumber'],
          endpoint: 'POST /api/orders',
          handler: 'productOrderController.js:createOrder',
          component: 'Order Fulfillment Controller & MySQL Orders Database',
          codeReference: 'DemoApp/backend/src/controllers/productOrderController.js:createOrder'
        },
        occurrences: purposeMismatchEvents.length,
        title: 'Purpose-Use Mismatch: Mobile Number Used Beyond Declared Scope',
        reason: `Declared purpose for mobileNumber in Data Inventory is 'OTP verification only', but observed in ${purposeMismatchEvents.length} order/login processing events inserting records into 'orders' table. DPDPA Section 6(1) requires processing only for declared, specified purposes.`,
    ...
    if (consentBreachEvents.length > 0) {
      ...
      const violationDoc = {
        ...
        breachLocation: {
          originType: 'CONSENT_STORE / EVENT_BUS',
          table: 'consents, consent_history, data_processing_events',
          columns: ['purpose', 'status', 'withdrawnAt', 'dataFields'],
          endpoint: 'POST /api/consents/:id/withdraw -> Background Dispatch',
          handler: 'consentController.js:withdrawConsent',
          component: 'Marketing Campaign Dispatch Worker & Consent Store',
          codeReference: 'DemoApp/backend/src/controllers/consentController.js'
        },
        occurrences: consentBreachEvents.length,
        title: 'Consent Enforcement Failure: Marketing Processing Post-Withdrawal',
        reason: `Marketing processing event executed for user who previously triggered CONSENT_WITHDRAWN in table 'consents'. Under DPDPA Section 6(4), Data Principal has the right to withdraw consent and processing must cease.`,
    ...
    if (retentionBreachEvents.length > 0) {
      ...
      const violationDoc = {
        ...
        breachLocation: {
          ...
          endpoint: 'DATABASE STORAGE: data_processing_events',
          handler: 'MySQL Database Storage Engine',
          component: 'Historical Event Store (MySQL Table: data_processing_events)',
          codeReference: 'DemoApp/backend/database/schema.sql (data_processing_events)'
        },
        occurrences: retentionBreachEvents.length,
        title: `Data Retention Threshold Exceeded (${ageDays} Days vs 365-Day Schedule)`,
        reason: `Marketing data record in table 'data_processing_events' is ${ageDays} days old, exceeding declared statutory retention limit of 365 days. DPDPA Section 8(7) requires erasing personal data upon expiry of the specified purpose.`,
  ...
  try {
    ...
    const report = {
      ...
      statutoryFindings: violations.map((v, i) => ({
        ...
        breachLocation: v.breachLocation || {
          originType: v.source === 'APPLICATION_LOG' ? 'APPLICATION_LOG' : 'DATABASE_TABLE',
          table: v.source === 'APPLICATION_LOG' ? 'application_audit_logs' : 'data_processing_events',
          columns: v.detectedData || v.detectedPII || [],
          endpoint: v.endpoint || '/api',
          handler: v.service || 'applicationController.js',
          component: `${v.service || 'Target Application'} Storage / API`
        },
        observedEvidence: {
          source: v.source,
          endpoint: v.endpoint,
          ...
          table: v.breachLocation?.table || (v.source === 'APPLICATION_LOG' ? 'application_audit_logs' : 'data_processing_events'),
          columns: v.breachLocation?.columns || v.detectedData || v.detectedPII || [],
          originType: v.breachLocation?.originType || v.source,
          component: v.breachLocation?.component || v.service,
          handler: v.breachLocation?.handler || 'Controller Handler'
        },
